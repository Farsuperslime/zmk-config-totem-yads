...
CACHE_PATHS = ("modules/", "tools/", "zephyr/", "bootloader/", "zmk/")
EXPIRE_IN = "1 month"
...
def build_job(entry: dict, args, module_mode: bool) -> dict:
    ...
    if module_mode:
        # Modules build from a west workspace outside the module checkout, with
        # the checkout itself passed in through ZMK_EXTRA_MODULES.
        west_root = "${TMPDIR:-/tmp}/zmk-config"
...
def generate(args) -> str:
    ...
    workspace = "${TMPDIR:-/tmp}/zmk-config (module layout)" if module_mode else "$CI_PROJECT_DIR"
    header = [
